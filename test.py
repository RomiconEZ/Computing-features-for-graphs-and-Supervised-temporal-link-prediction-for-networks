...
with open(output_path, 'w') as out:
    ...
    out.writelines(f"Близкие соседи: {common_neighbours(0, 1, staticGraph.adjacency_dict_of_dicts)}\n")
    out.writelines(f"Адамик-Адар: {adamic_adar(0, 1, staticGraph.adjacency_dict_of_dicts)}\n")
    out.writelines(f"Коэффициент Джаккарда: {jaccard_coefficient(0, 1, staticGraph.adjacency_dict_of_dicts)}\n")
    out.writelines(f"Преимущественная привязанность: {preferential_attachment(0, 1, staticGraph.adjacency_dict_of_dicts)}\n")